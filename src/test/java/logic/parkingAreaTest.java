...
public class parkingAreaTest {
	...
	public void BeforeTest() {
		...
		try {
			slot1 = new ParkingSlot("testS1", ParkingSlotStatus.FREE, StickersColor.RED, new MapLocation(0, 0));
			slot2 = new ParkingSlot("testS2", ParkingSlotStatus.FREE, StickersColor.RED, new MapLocation(0, 0));

			final Set<ParkingSlot> slots = new HashSet<ParkingSlot>();
			slots.add(slot1);
			slots.add(slot2);

			area = new ParkingArea("t1", new MapLocation(0, 0), slots, StickersColor.RED);
	...
	public void testAddRemoveSlots() {
		try {
			...
			final ParkingSlot slot3 = new ParkingSlot("testS2", ParkingSlotStatus.FREE, StickersColor.RED,
					new MapLocation(0, 0));
	...
	public void testGetSlotsByStatus() {
		...
		try {
			...
			if (areaList == null || areaList.isEmpty())
				throw new RuntimeException("There should be an area with areaId=" + 0);
			final Set<ParkingSlot> slotsByStatus = new ParkingArea(areaList.get(0))
					.getSlotsByStatus(ParkingSlotStatus.FREE);
